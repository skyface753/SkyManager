...
const createUser = "CREATE TABLE `user` ( `Name` varchar(50) NOT NULL,`email` varchar(50) NOT NULL, `Passwort` char(60) NOT NULL, `LastLogin_Date` date NOT NULL, `LastLogin_Time` time NOT NULL, `role_fk` varchar(50) NOT NULL, `isActive` int(11) NOT NULL, `lastTickets` varchar(250) NULL, `TOTPkey` varchar(250) NULL, `TOTPenabled` int(11) NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;"
...
const createTOTP = "CREATE TABLE `totp` (`ID` int(11) NOT NULL, `secret` varchar(250) NOT NULL, `issuer` varchar(250) NOT NULL, `algorithm` varchar(50) NOT NULL, `digits` int(11) NOT NULL, `period` int(11) NOT NULL, `customer_fk` int(11) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;"
const TableSQL = createKunden + createFirstKunde + createPasses + createRoles + createFirstRoles + createEintraege + createFirstEintraege + createTickets + createFirstTicket + createZustaende + createFirstZustaende + createUser + createFirstUser + createTaks + createFirstTask + createUsersTasks + createFirstUsersTasks + createWiki + createFirstWiki + createNotifications + createFirstNotifications + createDocs + createTOTP;
// INDIZIES 7
...
const indTOTP = "ALTER TABLE `totp` ADD PRIMARY KEY (`ID`), ADD KEY `customer_fk` (`customer_fk`);"
const indSQL = indKunden + indPasses + indRoles + indEintraege + indTickets + indZustaende + indUser + indTasks + indUsersTasks + indWiki + indNotifications + indDocs + indTOTP;
// Auto Incerement 5
...
const aiTOTP = "ALTER TABLE `totp` MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;"
const aiSQL = aiKunden + aiPasses + aiEintraege + aiTickets + aiZustaende + aiTasks + aiUsersTasks + aiWiki + aiNotifications + aiDocs + aiTOTP;
// Constraints (FK) 4
...
const fkTOTP = "ALTER TABLE `totp` ADD CONSTRAINT `totp_ibfk_1` FOREIGN KEY (`customer_fk`) REFERENCES `kunden` (`ID`);"

const fkSQL = fkPasses + fkEintraege + fkTickets + fkUser + fkTasks + fkUsersTasks + fkNotifications + fkDocs + fkTOTP;
